...
* [**noVNC**](https://github.com/novnc/noVNC) - HTML5 VNC client (default http port `7860`)
* Browsers:
  * Mozilla Firefox
  * Chromium
...
- Run command with mapping to local port `5901` (vnc protocol) and `7860` (vnc web access):

      docker run -d -p 5901:5901 -p 7860:7860 consol/rocky-xfce-vnc

- Change the default user and group within a container to your own with adding `--user $(id -u):$(id -g)`:

      docker run -d -p 5901:5901 -p 7860:7860 --user $(id -u):$(id -g) consol/rocky-xfce-vnc

- If you want to get into the container use interactive mode `-it` and `bash`

      docker run -it -p 5901:5901 -p 7860:7860 consol/rocky-xfce-vnc bash
...
* connect via __noVNC HTML5 full client__: [`http://localhost:7860/vnc.html`](http://localhost:7860/vnc.html), default password: `vncpassword`
* connect via __noVNC HTML5 lite client__: [`http://localhost:7860/?password=vncpassword`](http://localhost:7860/?password=vncpassword)
...
Add the `--user` flag to your docker run command:
    ...
    docker run -it --user 0 -p 6911:7860 consol/rocky-xfce-vnc

#### 2.2) Using user and group id of host system
Add the `--user` flag to your docker run command:

    docker run -it -p 6911:7860 --user $(id -u):$(id -g) consol/rocky-xfce-vnc
...
the docker run command:
    ...
    docker run -it -p 5901:5901 -p 7860:7860 -e VNC_RESOLUTION=800x600 consol/rocky-xfce-vnc
...
If you open some graphic/work intensive websites in the Docker container (especially with high resolutions e.g. `1920x1080`) it can happen that Chromium crashes without any specific reason. The problem there is the too small `/dev/shm` size in the container. Currently there is no other way, as define this size on startup via `--shm-size` option, see [#53 - Solution](https://github.com/ConSol/docker-headless-vnc-container/issues/53#issuecomment-347265977):
    ...
    docker run --shm-size=256m -it -p 7860:7860 -e VNC_RESOLUTION=1920x1080 consol/rocky-xfce-vnc chromium-browser http://map.norsecorp.com/
